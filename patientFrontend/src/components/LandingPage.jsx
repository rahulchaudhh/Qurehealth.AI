...
export default function LandingPage() {
    ...
    return (
        <div className="min-h-screen bg-white font-sans text-slate-900 selection:bg-indigo-100 selection:text-indigo-900 overflow-x-hidden relative">
                    ...
                    <div className={`backdrop-blur-xl bg-white/70 border border-white/50 shadow-sm rounded-full px-6 py-3 flex justify-between items-center transition-all duration-300 ${scrolled ? 'shadow-lg bg-white/90' : ''}`}>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                className="hidden sm:flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-slate-600 hover:text-indigo-600 border border-slate-200 hover:border-indigo-300 rounded-full transition-all"
                            ...
                            <button type="button" onClick={handleLogin}
                                className="hidden sm:block px-5 py-2 text-sm font-medium text-slate-700 hover:text-indigo-600 transition-colors cursor-pointer hover:bg-slate-50 rounded-full">
                                {t.login}
                            </button>
                            <button type="button" onClick={handleRegister}
                                className="hidden sm:block px-5 py-2 bg-slate-900 text-white text-sm font-semibold rounded-full hover:bg-slate-800 transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5 cursor-pointer">
                                {t.getStarted}
                            </button>
                        ...
                        <div className="md:hidden mt-2 mx-2 bg-white/95 backdrop-blur-xl border border-white/60 rounded-3xl shadow-2xl p-5 flex flex-col gap-1">
                            ...
                            <div className="border-t border-slate-100 mt-2 pt-3 flex flex-col gap-2">
                                ...
                                <button type="button" onClick={() => { handleLogin(); setMobileOpen(false); }}
                                    className="px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50 rounded-xl transition-all text-left">
                                    {t.login}
                                </button>
                                <button type="button" onClick={() => { handleRegister(); setMobileOpen(false); }}
                                    className="px-4 py-2.5 bg-slate-900 text-white text-sm font-semibold rounded-xl hover:bg-slate-800 transition-all text-center">
                                    {t.getStarted}
                                </button>
                            </div>
                        </div>
            ...
            {/* ══════════════ FLOATING WHATSAPP / VIBER CTA ══════════════ */}
            <a
                href="https://wa.me/9779817831552?text=Hello%20Qurehealth.AI%2C%20I%27d%20like%20to%20know%20more."
                target="_blank"
                rel="noopener noreferrer"
                className="fixed bottom-6 right-6 z-[9998] flex items-center gap-2 px-4 py-3 bg-green-500 hover:bg-green-600 text-white text-sm font-semibold rounded-full shadow-2xl hover:shadow-green-300/50 transition-all hover:-translate-y-1 group"
