...
import api from '../../api/axios'; // Doctor's axios instance

const ChatModal = ({ isOpen, onClose, appointmentId, patientName, patientImage }) => {
      ...
      if (res.data.success) {
        ...
        if (isInitial) {
          ...
          const unreadIds = data
            .filter(m => m.senderModel === 'Patient' && !m.isRead)
            .map(m => m._id);
        ...
        <div className="bg-white px-5 py-4 border-b border-gray-100 flex items-center justify-between shrink-0 h-[72px]">
          <div className="flex items-center gap-3">
            <div className="relative">
              {patientImage ? (
                <img src={patientImage} alt="Patient" className="w-10 h-10 rounded-full object-cover border border-gray-200" />
              ) : (
            ...
            <div>
              <h3 className="font-bold text-gray-900 leading-none">{patientName}</h3>
          ...
          {loading ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-gray-400 gap-3">
              ...
              <p className="text-xs text-gray-400 text-center max-w-[250px]">
                Send a message to {patientName} regarding their appointment.
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="flex justify-center mb-2">
                <span className="bg-blue-50/80 text-blue-600/80 text-[10px] font-medium px-4 py-1.5 rounded-full border border-blue-100/50 flex items-center gap-1.5">
                  <Clock size={10} />
                  Chat is securely encrypted and saved to patient's medical history
                </span>
              </div>
              ...
              {messages.map((msg, index) => {
                ...
                const showAvatar = !isMe && (!prevMsg || prevMsg.senderModel !== 'Patient');
                    ...
                    <div className="flex items-end gap-2 max-w-[75%]">
                      ...
                      {!isMe && (
                        <div className="w-7 h-7 flex-shrink-0">
                          {showAvatar && (
                            patientImage ? (
                              <img src={patientImage} alt="Pt" className="w-7 h-7 rounded-full object-cover" />
                            ) : (
